...
async def setup_db():
    """Initialize database and clean up after tests"""
    await init_db()
    yield
    # Clean up
...
@pytest.mark.asyncio
async def test_get_user_success(event_loop):
    user_data = UserCreate(
        username="testuser",
        full_name="Test User",
        email="test@example.com"
...
async def test_get_nonexistent_user():
    ...
    response = client.get(f"/v1/users/{nonexistent_id}")
...
async def test_get_user_invalid_id():
    ...
    response = client.get(f"/v1/users/{invalid_id}")
    assert response.status_code == 404
